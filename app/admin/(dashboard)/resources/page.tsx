        ...
        {resources && resources.length > 0 && (
          <div className="bg-white border border-aether-electric-teal/10 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {resources.map((r) => (
                  <tr key={r.id} className="border-b border-aether-electric-teal/5 last:border-0 hover:bg-aether-electric-teal/5 transition-colors">
                    <td className="px-4 py-3 font-medium text-deep-ink">{r.title}</td>
                    <td className="px-4 py-3 text-deep-ink/70">{r.resource_type}</td>
                    <td className="px-4 py-3 text-deep-ink/70">{r.category || '—'}</td>
