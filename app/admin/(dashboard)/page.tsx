...
export default async function AdminDashboard() {
  ...
  const statusChartData = [
    { label: 'Received', value: byStatus.received, color: '#3388FF' },
    { label: 'In Progress', value: byStatus.in_progress, color: '#8DB0E2' },
    { label: 'Completed', value: byStatus.completed, color: '#0B428E' },
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-deep-ink/60">Leads overview, based on live data</p>
        </div>
        <Link
          href="/admin/leads"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statCards.map((stat) => {
          ...
          return (
            <div key={stat.label} className="bg-white border border-aether-electric-teal/10 rounded-xl p-5">
              <div className="w-11 h-11 rounded-lg bg-aether-electric-teal/10 flex items-center justify-center mb-4">
                <Icon size={20} className="text-aether-electric-teal" />
              </div>
              <div className="text-2xl font-bold text-deep-ink">{stat.value}</div>
              <div className="flex items-center justify-between mt-1">
                <span className="text-xs text-deep-ink/60">{stat.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white border border-aether-electric-teal/10 rounded-xl p-5">
          <h2 className="font-display font-bold text-aether-deep-teal mb-4">Leads, Last 14 Days</h2>
          <MiniAreaChart data={chartData} />
        </div>
        <div className="bg-white border border-aether-electric-teal/10 rounded-xl p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-aether-electric-teal/10 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-aether-electric-teal/10">
            ...
            <table className="w-full text-sm">
              <tbody>
                {recentLeads.map((l: any) => (
                  <tr key={l.id} className="border-b border-aether-electric-teal/5 last:border-0">
                    <td className="px-5 py-3">
                      <Link href={`/admin/leads/${l.id}`} className="font-medium text-deep-ink hover:text-aether-electric-teal">
                        Lead #{l.id.slice(0, 8)}
                      </Link>
                    </td>
                    <td className="px-5 py-3 text-deep-ink/60">
          ...
          <div className="space-y-2">
            {[
              ...
              <Link
                key={action.href}
                href={action.href}
                className="block px-3 py-2.5 rounded-lg text-sm font-medium text-deep-ink hover:bg-aether-electric-teal/10 hover:text-aether-electric-teal transition-colors"
              >
                {action.label}
              </Link>
            ))}
