...
export default async function EditServicePage({
  ...
  return (
    <FormWrapper title="Edit Service" backHref="/admin/services">
      ...
      <div className="mt-10 pt-8 border-t border-aether-electric-teal border-opacity-10">
        ...
        {linkedTiers && linkedTiers.length > 0 && (
          <ul className="space-y-2">
            {linkedTiers.map((t) => (
              <li
                key={t.id}
                className="flex items-center justify-between bg-sky-white border border-aether-electric-teal border-opacity-10 rounded-lg px-4 py-3 text-sm"
              >
