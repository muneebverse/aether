  ...
  return (
    <nav
      className={`sticky top-0 z-50 transition-[background-color,border-color,box-shadow] duration-300 ${
        scrolled
          ? 'bg-sky-white/90 backdrop-blur-md border-b border-aether-electric-teal/10 shadow-[0_1px_0_rgba(11,66,142,0.06)]'
      ...
      <div className="container-aether flex items-center justify-between h-16">
        ...
        <div className="hidden lg:flex items-center gap-0.5">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`relative px-3 py-2 text-sm font-medium transition-colors rounded-full whitespace-nowrap ${
                  active
          ...
          <Magnetic strength={0.2}>
            <Link href="/contact" className="btn btn-primary ml-3 whitespace-nowrap">
              Contact Us
            </Link>
          </Magnetic>
        </div>
        ...
        <button
          ...
          className="lg:hidden text-aether-deep-teal p-2 -mr-2"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden border-t border-aether-electric-teal/10 bg-sky-white"
          >
